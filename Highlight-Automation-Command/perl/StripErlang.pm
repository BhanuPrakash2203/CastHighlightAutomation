...
# kinds: 
#  -  Single quote string   ' '
#  -  Double quote string   " " may be multiline strings
#
#
# Escape :
#     respectively  \' \" 
...
sub StripErlang($$$$)
{
    ...
    configureLocalTraces('StripErlang', $options); # traces_filter_line
    my $StripErlangTiming = new Timing ('StripErlang', Timing->isSelectedTiming ('Strip'));
    $StripErlangTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $StripErlangTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    $StripErlangTiming->dump('StripErlang') if ($b_timing_strip); # timing_filter_line
    return 0;
}
