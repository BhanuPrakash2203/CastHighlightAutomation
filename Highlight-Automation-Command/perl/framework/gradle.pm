...
sub parseCompile($) {
	...
	### compile ("group", "name", "version")
		...
		framework::Logs::Warning("Unable to decode compile parameters format : ". substr($$buffer, pos($$buffer), 20). "...\n");
	}
...
sub parseInstruction($) {
	...
	### PARSE SPECIFIC PATTERNS (contest dependent)
...
sub gradle($$$) {
	...
	# 1. scan all gradle files in project to list version values into ext {...}
	# 		path are sorted by alphabetically sort to respect inheritance in sharing version
	# 2. update version values considering gradle parent is in parent folder
	foreach my $file_gradle (@$files_gradle) {
		...
		if (basename($file_gradle) eq 'build.gradle') {
			...
			# Groovy syntax: closures
			if ($$code =~ /\bdependencies\s*\{/) {
				parseBloc(\$views{'code'});
			}
			# Groovy syntax: collections of librairies
		...
		# replace version if present in same file or in other gradle with <apply from> statement
