package StripTypescript;
use strict;
use warnings ;
use Carp::Assert; # traces_filter_line
use Timing ; # timing_filter_line
use StripUtils;

# prototypes publics
sub StripTypescript($$$$);

use StripUtils qw(
                  garde_newlines
                  warningTrace
                  configureLocalTraces
                  StringStore
                  dumpVueStrings
                 );

StripUtils::init('StripTypescript', 0);

#-----------------------------------------------------------------------

my $blank = \&garde_newlines;

my $TEXT;
my $CODE="";
my $COMMENT = "";
my $MIX = "";

my $STRING_CONTEXT = { 
	'nb_distinct_strings' => 0,
	'strings_values' => {},
	'strings_counts' => {}
} ;
my $RT_STRINGS = $STRING_CONTEXT->{'strings_values'};


# COMMENTS FEATURE :
#-------------------
# kinds :
#   - Simple line : // ..... \n
#   - Multiline   : /* ..... */
sub parseLineComment($) {
	my $el = shift;
	my $spaces = $blank->($el);
	$CODE .= $spaces;
	$COMMENT .= $el;
	$MIX .= '/*';
	
	while ( $$TEXT =~ /\G(\n|[^\n]*)/gc) {
		$spaces = $blank->($1);
		if ($1 eq "\n") {
			$CODE .= $blank->($1);
			$COMMENT .= $1;
			$MIX .= "*/\n";
			return 0;
		}
		else {
			$CODE .= $blank->($1);
			$COMMENT .= $1;
			$MIX .= $1;
		}
	}
	print STDERR "Unterminated comment !!!\n";
	
	return 1;
}

sub parseMultilineLineComment($) {
	my $el = shift;
	my $spaces = $blank->($el);
	$CODE .= $spaces;
	$COMMENT .= $el;
	$MIX .= $el;
	
	while ( $$TEXT =~ /\G(\n|\*\/|\*|[^*\n]*)/gc) {
		$spaces = $blank->($1);
		if ($1 eq "*/") {
			$CODE .= $blank->($1);
			$COMMENT .= $1;
			$MIX .= $1;
			return 0;
		}
		elsif( $1 eq "\n") {
			$CODE .= $blank->($1);
			$COMMENT .= "\*\/\n\/\*";
			$MIX .= "\*\/\n\/\*";
		}
		else {
			$CODE .= $blank->($1);
			$COMMENT .= $1;
			$MIX .= $1;
		}
	}
	print STDERR "Unterminated comment !!!\n";
	return 1;
}

# STRINGS FEATURES
#-----------------
# kinds: 
#  -  Simple quote string  ''
#  -  Double quote string  ""
#  -  Template string      ``
#
# Escape :
#     respectively \' \" \`
#
# Line Continuation :
#     allowed for all

# DOUBLE QUOTE
sub parseDblString($) {
	my $el = shift;
	my $spaces = $blank->($el);
	$COMMENT .= $spaces;
	$MIX .= $spaces;
	my $string_buffer = $1;
	my $CODE_padding = "";
	
	while ( $$TEXT =~ /\G(\\"|\"\"|"|\\\\|\\|\\\n|[^\\"]*)/gc) {
		$spaces = $blank->($1);
		$CODE_padding .= $spaces;
		$COMMENT .= $spaces;
		# $MIX .= $spaces;
		
		if ($1 eq '"') {
			$string_buffer .= $1;
			my $string_id = StringStore( $STRING_CONTEXT, $string_buffer );
			$CODE .= "$string_id".$CODE_padding;
			$MIX .= "$string_id".$CODE_padding;
			return 0;
		}
		elsif ($1 ne "\\\n") {
			# not a continuing line => update $string_buffer
			$string_buffer .= $1;
		}
	}
	print STDERR "Unterminated string !!!\n";
	print STDERR "$string_buffer\n";
	return 1;
}

# SIMPLE QUOTE
sub parseSplString($) {
	my $el = shift;
	my $spaces = $blank->($el);
	$COMMENT .= $spaces;
	$MIX .= $spaces;
	my $string_buffer = $1;
	my $CODE_padding = "";
	
	while ( $$TEXT =~ /\G(\\'|\'\'|'|\\\\|\\|\\\n|[^\\']*)/gc) {
		$spaces = $blank->($1);
		$CODE_padding .= $spaces;
		$COMMENT .= $spaces;
		# $MIX .= $spaces;
		
		if ($1 eq "'") {
			$string_buffer .= $1;
			my $string_id = StringStore( $STRING_CONTEXT, $string_buffer );
			$CODE .= "$string_id".$CODE_padding;
			$MIX .= "$string_id".$CODE_padding;
			return 0;
		}
		elsif ($1 ne "\\\n") {
			# not a continuing line => update $string_buffer
			$string_buffer .= $1;
		}
	}
	print STDERR "Unterminated string !!!\n";
	return 1;
}

# TEMPLATE
sub parseTplString($) {
	my $el = shift;
	my $spaces = $blank->($el);
	$COMMENT .= $spaces;
	my $string_buffer = $1;
	my $CODE_padding = "";
	
	my $inside_code = 0;
	
	while ( $$TEXT =~ /\G(\\`|`|\\\\|\\|\n|\\\n|\$\{|\$|\{|\}|[^\\`\n\$\{\}]*)/gc) {
		$spaces = $blank->($1);
		$COMMENT .= $spaces;
		$CODE_padding .= $spaces;
		
		if ($inside_code) {
			if ($1 eq '${') {
				$inside_code++;
			}
			elsif ($1 eq '{') {
				$inside_code++;
			}
			elsif ($1 eq '}') {
				$inside_code--;
			}
			$string_buffer .= $1;
		}
		elsif ($1 eq '${') {
			$inside_code++;
		}
		elsif ($1 eq "`") {
			$string_buffer .= $1;
			my $string_id = StringStore( $STRING_CONTEXT, $string_buffer );
			$CODE .= "$string_id".$CODE_padding;
			$MIX .= "$string_id".$CODE_padding;
#print "TEMPLATE STRING : $string_buffer\n";
			return 0;
		}
		elsif ($1 eq "\n") {
			$string_buffer .= $1;
		}
		elsif ($1 ne "\\\n") {
			# not a continuing line => update $string_buffer
			$string_buffer .= $1;
		}
		else {
			# $string_buffer not updated
		}
	}
	print STDERR "Unterminated string !!!\n";
	return 1;
}

sub parseRegExp($$) 
{
    my $openingSymbol = shift;
    my $textInput = shift;

    my $regexp = $openingSymbol;
    # $CODE .= $openingSymbol;
    # $MIX .= $openingSymbol;

    my $closingSymbol;
    $closingSymbol = '/' if ($openingSymbol eq '/');
    
    my $matchClosingSymbol = "\\".$closingSymbol;
    my $matchOpeningSymbol = "\\".$openingSymbol;
    my $matchOpeningEscapedSymbol = "\\\\\\".$openingSymbol;
    my $matchClosingEscapedSymbol = "\\\\\\".$closingSymbol;
    # print 'openingSymbol=<'.$openingSymbol.">\n";
    # print 'closingSymbol=<'.$closingSymbol.">\n";
    # print 'matchOpeningSymbol='.$matchOpeningSymbol."\n";
    # print 'matchClosingSymbol='.$matchClosingSymbol."\n";
    # print 'matchOpeningEscapedSymbol='.$matchOpeningEscapedSymbol."\n";
    # print 'matchClosingEscapedSymbol='.$matchClosingEscapedSymbol."\n";
    
    my $countSymbol = 0;
    my $insideBracket = 0;
    while ($$textInput =~ /
    \G(
        $matchOpeningEscapedSymbol
        |$matchClosingEscapedSymbol
        |$matchOpeningSymbol
        |$matchClosingSymbol
        |\\\\
        |\\\/
        |\\\[
        |\\\]
        |\\
        |\/
        |\[
        |\]
        |[^\[\]\/$matchOpeningSymbol$matchClosingSymbol\\]+
    )/xgc)
    {
		
#print "ITEM : $1 \t($insideBracket)\n";
		if ($insideBracket) {
			if ($1 eq "]") {
				$insideBracket = 0;
			}
			$regexp .= $1;
		}
		elsif ($1 eq "[") {
			$insideBracket = 1;
			$regexp .= $1;
		}
        elsif ($1 eq $closingSymbol and $countSymbol == 0) 
        {
            #$CODE .= $1;
            #$MIX .= $1;
            $regexp .= $1;
            
            # consume modifiers :
            if ($$textInput =~ /\G([gimsyu])/gc) {
				$regexp .= $1;
			}
                        
#print 'Regexp detected <'.$regexp.">\n";
            my $padding = " " x ( () = $regexp =~ /\n/g);
            $CODE .= "REGEXP_XX".$padding;
            $MIX .= "REGEXP_XX".$padding;
            return 0;
        }
        # elsif ($1 eq '[')
        # {
        # # print '++countSymbol' ."\n";
            # $CODE .= $1;
            # $MIX .= $1;
            # $regexp .= $1;
            # $countSymbol++;            
        # }
        # elsif ($1 eq ']')
        # {
        # # print '--countSymbol' ."\n";
            # $CODE .= $1;
            # $MIX .= $1;
            # $regexp .= $1;
            # $countSymbol--;            
        # }
        else
        {
            #$CODE .= $1;
            #$MIX .= $1;
            $regexp .= $1;
       }
    }
    
    print STDERR "[parseRegExp] Missing closing delimiter $closingSymbol \n";
    return 1;
}

sub parseCode($$) {
	$TEXT = shift;
	my $options = shift;
	
	my $err = 0;
	
	# Init data
	$CODE = "";
    $COMMENT = "";
    $MIX = "";
    $STRING_CONTEXT = { 
		'nb_distinct_strings' => 0,
		'strings_values' => {},
		'strings_counts' => {}
	};
	$RT_STRINGS = $STRING_CONTEXT->{'strings_values'};
	
	# characters that can be found before the "/" of a regexp ...
    my $prefixRegexOperators =  qr /\=|\(|\[|\||\&|\!|:|>|,/;
    my $previousItem = "";
    my $regexp_possible = 0;
    
	while ( $$TEXT =~ /
    \G
    (
        ($prefixRegexOperators)
        |\/\/
        |\/\*
        |\/
        |\=
        |\(
        |\[
        |"
        |'
        |`
        |\|
        |\&
        |\!
        |:
        |>
        |,
        |[^"'\/`\=\(\[\|\&\!:>,]*
    )
    /xgc) 
    {
        my $spaces = $blank->($1);
    
        my $pattern = $1;
        my $RegexBegin = $2; # pattern that can be followed by a regexp
#print "ITEM = $1 --- $regexp_possible\n";
		if ($1 eq '//') {
			$err = parseLineComment($1);
		}
		elsif ($1 eq '/*') {
			$err = parseMultilineLineComment($1);
		}
		elsif ($1 eq '"') {
			$err = parseDblString($1);
			$regexp_possible = 0;
		}
		elsif ($1 eq "'") {
			$err = parseSplString($1);
			$regexp_possible = 0;
		}
		elsif ($1 eq "`") {
			$err = parseTplString($1);
			$regexp_possible = 0;
		}
		elsif (defined $RegexBegin) 
		{
			$regexp_possible = 1;
			
			if ($RegexBegin eq "!") {
				my $CaptureLength = length $pattern;
				
				my $charBeforeBang = substr ($$TEXT, pos($$TEXT)-$CaptureLength-1, 1);
				if ($charBeforeBang =~ /\w/)  {
					# do not parse a regexp if the ! sign is the "defined assertion" applying to a data.
					$regexp_possible = 0;
				}
			}
			
			$CODE .= $1;
			$COMMENT .= $spaces;
            $MIX .= $1;
		}
        elsif ($1 eq "/")
        {
            if (	($regexp_possible) || ($previousItem =~ /(?:\breturn|\+|-|\*)\s*\z/) ||    # "/" is preceded by these items ... 
					($previousItem =~ /[^\w\)]\s*$/) ||		# "/" is not preceded by an identifier or a ")" (would mean it is the / or /= arithmetic operator)
					($$TEXT =~ /(?=\G\s*[^\s\(\w\$])/gc)  	# "/" is not followed by "(", \w or $ ...
				)
            {
				$err = parseRegExp("/",$TEXT);
				$regexp_possible = 0;
            }
            else {
				$CODE .= $1;
				$COMMENT .= $spaces;
				$MIX .= $1;
			}
        }
		else {
			$CODE .= $1;
			$COMMENT .= $spaces;
            $MIX .= $1;
            
			# the pattern was not able to introduce a regex => reset flag
			if ($pattern =~ /\S/) {
				$regexp_possible = 0;
			}
		}
		
		if ($err gt 0) {
			return 1;
		}
		
		$previousItem = $pattern;
	}
	
	return  0;
}

# analyse du fichier
sub StripTypescript($$$$)
{
    my ($filename, $vue, $options, $couples) = @_;
    my $b_timing_strip = ((defined Timing->isSelectedTiming ('Strip'))? 1 : 0);

    configureLocalTraces('StripTypescript', $options); # traces_filter_line
    my $stripTypescriptTiming = new Timing ('StripTypescript', Timing->isSelectedTiming ('Strip'));
    $stripTypescriptTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $stripTypescriptTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line

    $vue->{'comment'} = $COMMENT;
    $vue->{'HString'} = $RT_STRINGS;
    $vue->{'code_with_prepro'} = "";
    $vue->{'MixBloc'} = $MIX;
    $vue->{'agglo'} = "";                       
    StripUtils::agglomerate_C_Comments(\$MIX, \$vue->{'agglo'});
    $vue->{'MixBloc_LinesIndex'} = StripUtils::createLinesIndex(\$MIX);
    $vue->{'agglo_LinesIndex'} = StripUtils::createLinesIndex(\$vue->{'agglo'});    
    $vue->{'code'} = $CODE;
    
    if ( defined $options->{'--dumpstrings'})
    {
      dumpVueStrings(  $RT_STRINGS , $STDERR );
    }
    
	# if (($filename ne AnaUtils::DUMMYFILENAME) and defined $options->{'--strip'})
	# {
		# Vues::dump_vues( $filename, $vue, $options);
	# }
	
    if ( $err gt 0) {
      my $message = 'Fatal error when separating code/comments/strings';
      Erreurs::LogInternalTraces ('erreur', undef, undef, 'STRIP // ABORT_CAUSE_SYNTAX_ERROR', $message);
      return Erreurs::FatalError( Erreurs::ABORT_CAUSE_SYNTAX_ERROR, $couples, $message);
    }

    $stripTypescriptTiming->dump('StripTypescript') if ($b_timing_strip); # timing_filter_line
    return 0;
}

1; # Le chargement du module est okay.

