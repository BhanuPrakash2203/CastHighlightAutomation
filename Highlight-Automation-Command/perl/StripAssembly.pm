...
StripUtils::init('StripAssembly', 0);
...
sub parseCode($$) {
	...
	while ( $$TEXT =~ /\G(\/[^\/]|;|"|'|[^"';]+)/gc) {
        ...
        if ($1 eq '#' or $1 eq ';' or $1 eq '@' or $1 eq '//' or $1 eq '|' or $1 eq '!') {
...
sub StripAssembly($$$$)
{
    ...
    configureLocalTraces('StripAssembly', $options); # traces_filter_line
    my $stripAssemblyTiming = new Timing ('StripAssembly', Timing->isSelectedTiming ('Strip'));
    $stripAssemblyTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $stripAssemblyTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    $stripAssemblyTiming->dump('StripAssembly') if ($b_timing_strip); # timing_filter_line
    return 0;
}
