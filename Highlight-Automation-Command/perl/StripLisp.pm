...
# sub parseSplString($) {
	...
	# while ( $$TEXT =~ /\G(\\'|\'\'|'|\\|\\\n|[^\\']*)/gc) {
		...
		# }
	# }
	# print STDERR "Unterminated string parseSplString!!!\n";
	# return 1;
# }
...
sub StripLisp($$$$)
{
    ...
    configureLocalTraces('StripLisp', $options); # traces_filter_line
    my $stripLispTiming = new Timing ('StripLisp', Timing->isSelectedTiming ('Strip'));
    $stripLispTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $stripLispTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    $stripLispTiming->dump('StripLisp') if ($b_timing_strip); # timing_filter_line
    return 0;
}
