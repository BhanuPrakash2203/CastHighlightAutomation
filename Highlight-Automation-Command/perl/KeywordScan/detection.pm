...
# sub cleanFilesDetections($) {
	...
	# my $file_idx = 0;
    # # my @indexFilesToDelete;
    # my @keywordsToDelete;

    # ###################
    # # may be drastic (list of files would not be complete) 
            # # # clean file list
            # # for my $file (@TAB_FILES) {
                
                # # my $bool_clean_file = 0;
                # # my $values = $detector->getFileValues($file_idx);
                
                # # # associate keywords values
                # # for my $keyword (@{$TAB_DETECTED_KEYWORDS}) {
                    # # if (defined $values->{$keyword} and $values->{$keyword} > 0) {
                        # # $bool_clean_file = 1;
                        # # last;
                    # # }
                # # }
            
                # # if (defined $bool_clean_file  and $bool_clean_file == 0)
                # # {
                    # # # print "CLEAN the file $file\n";
                    # # push (@indexFilesToDelete, $file_idx);
                # # }
                
                # # $file_idx++;
            # # }
            
            # # for my $indexFile (@indexFilesToDelete)
            # # {
                # # splice(@TAB_FILES, $indexFile, 1);
            # # }
    # ###################
	...
	# for my $keyword (@{$TAB_DETECTED_KEYWORDS}) {
        ...
        # if (defined $bool_clean_kw  and $bool_clean_kw == 0)
        # {
        # push (@keywordsToDelete, $keyword);
        # }
	# }
      
    # for my $keyword_to_delete (@keywordsToDelete)
    # {
        # $detector->deleteKeywordDetection($keyword_to_delete);
    # }
# }
...
# sub dumpAppliDetections($) {
	...
	# print $OUTPUT_FILE_HANDLER "\nAPPLI SECTION\n";
	# print $OUTPUT_FILE_HANDLER "PatternID;Found\n";

    # for my $detectorName (sort keys %APPLI_DATA) 
    # {
        # if ($detectorName eq $scanName)
        # {
            # for my $idPattern (sort keys %{$APPLI_DATA{$detectorName}}) {
               
                # # print patternID in column 0
                # print $OUTPUT_FILE_HANDLER "$idPattern;";
                
                # # print in column 1: 
                # # 	number of detection if a detection has been found
                # #	0 else
                # if ($APPLI_DATA{$detectorName}{$idPattern} > 0) {
                    # print $OUTPUT_FILE_HANDLER "Yes;";
                # }
                # else {
                    # print $OUTPUT_FILE_HANDLER "No;";
                # }
