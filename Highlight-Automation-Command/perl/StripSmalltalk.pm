...
StripUtils::init('StripSmalltalk', 0);
...
sub parseCode($$) {
    ...
    # exception for $' for not confusing with strings
    # exception for $" for not confusing with comments
	while ( $$TEXT =~ /\G(\$"|\$'|"|'|\$|[^"'\$]*)/gc) {
...
sub StripSmalltalk($$$$)
{
    ...
    configureLocalTraces('StripSmalltalk', $options); # traces_filter_line
    my $StripSmalltalkTiming = new Timing ('StripSmalltalk', Timing->isSelectedTiming ('Strip'));
    $StripSmalltalkTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $StripSmalltalkTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    $StripSmalltalkTiming->dump('StripSmalltalk') if ($b_timing_strip); # timing_filter_line
    return 0;
}
