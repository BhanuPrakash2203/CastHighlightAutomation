...
sub StripDB2($$$$)
{
    ...
    configureLocalTraces('StripDB2', $options); # traces_filter_line
    my $stripDB2Timing = new Timing ('StripDB2', Timing->isSelectedTiming ('Strip'));
    $stripDB2Timing->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $stripDB2Timing->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    if ( $err gt 0) {
     ...
     if ($CODE !~ /\S/s ) {
      return Erreurs::FatalError( Erreurs::ABORT_CAUSE_NOT_ENOUGH_CODE, $couples, "No DB2 found");
    }
    ...
    $stripDB2Timing->dump('StripDB2') if ($b_timing_strip); # timing_filter_line
    return 0;
}
