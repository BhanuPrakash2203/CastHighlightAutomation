...
StripUtils::init('StripPostgreSQL', 0);
...
sub StripPostgreSQL($$$$)
{
    ...
    configureLocalTraces('StripPostgreSQL', $options); # traces_filter_line
    my $stripPostgreSQLTiming = new Timing ('StripPostgreSQL', Timing->isSelectedTiming ('Strip'));
    $stripPostgreSQLTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $stripPostgreSQLTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    if ( $err gt 0) {
     ...
     if ($CODE !~ /\S/s ) {
      return Erreurs::FatalError( Erreurs::ABORT_CAUSE_NOT_ENOUGH_CODE, $couples, "No PostgreSQL found");
    }
    ...
    $stripPostgreSQLTiming->dump('StripPostgreSQL') if ($b_timing_strip); # timing_filter_line
    return 0;
}
