...
sub CountSwift($$$) {
    ...
    CloudReady::detection::addFileDetection($fichier, CloudReady::Ident::Alias_Blockchain, $checkPattern->(qr/\bimport SawtoothSigning\b|\bclass\s+Blockchain\b|\bBlockchain\b[\w.\s]*\(|\bstruct\s+Ethereum|\bEthereum[\w.\s]*\(/, $code));
