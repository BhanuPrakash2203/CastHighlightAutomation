...
sub checkHardCodedConnectionString($$$$$$) {
	...
	my $HardCodedConnectionString = 0;
	$HardCodedConnectionString = CloudReady::lib::ElaborateLog::ElaborateLogPartOne($View_code, $regexp_ConnectionStringBuilder, $mnemo, $rulesDescription);
	...
	if (defined $HardCodedConnectionString && $HardCodedConnectionString > 0) {
		my $SQLDatabaseUnsecureAuthentication;
		$SQLDatabaseUnsecureAuthentication = checkSQLDatabaseUnsecureAuthentication(\%HardCodedConnectionStrings, $View_text);
		CloudReady::detection::addFileDetection($fichier, CloudReady::Ident::Alias_SQLDatabaseUnsecureAuthentication(), $SQLDatabaseUnsecureAuthentication);
	}
...
sub CountDotnet($$$$) {
	...
	CloudReady::detection::addFileDetection($fichier, CloudReady::Ident::Alias_ApiUnsecured, $checkPattern->('\[\s*Authorize\b|\bAuthorizeAttribute\b|\bIsInRole\b', $code, CloudReady::Ident::Alias_ApiUnsecured));
	...
	CloudReady::detection::addFileDetection($fichier, CloudReady::Ident::Alias_HTTPProtocol(), checkContentString($code, $HString, qr/\bhttps?\:(?:\/\/)\b/i, $text, CloudReady::Ident::Alias_HTTPProtocol));
	# 09/06/2020 FTPProtocol
	CloudReady::detection::addFileDetection($fichier, CloudReady::Ident::Alias_FTPProtocol(), checkContentString($code, $HString, qr/\bs?ftps?\:(?:\/\/)\b/i, $text, CloudReady::Ident::Alias_FTPProtocol));
	# 09/06/2020 LDAPProtocol
	CloudReady::detection::addFileDetection($fichier, CloudReady::Ident::Alias_LDAPProtocol(), checkContentString($code, $HString, qr/\bldaps?\:(?:\/\/)\b/i, $text, CloudReady::Ident::Alias_LDAPProtocol));
	# 10/05/2021 HL-1724
