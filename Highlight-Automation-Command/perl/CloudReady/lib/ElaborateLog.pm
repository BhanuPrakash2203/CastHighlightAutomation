       ...
       while (<$handle>) {
           if (/$regex/) {
               ...
               if ($res == 0) {
                   ...
                   elsif ($technoLabelInPortal eq 'CCPP') {
                       $technoLabelInPortal = 'CPP'
                   }
...
sub SimpleElaborateLog ($$$$$) {
    ...
    if (defined $dbgMatchPatternDetail) {
        my $SHA = getSHA();
        if (exists $rulesDescription->{$mnemoID}->{$techno}) {
            $SCAN_LOG{$rulesDescription->{$mnemoID}->{$techno}->{$ruleNamePortal}->[0]}{$SHA}[0] .= "$numline\[\-\]\|";
            $SCAN_LOG{$rulesDescription->{$mnemoID}->{$techno}->{$ruleNamePortal}->[0]}{$SHA}[1] = $rulesDescription->{$mnemoID}->{$techno}->{$ruleNamePortal}->[1];
        }
    }
}
