    ...
    while ($$code =~ /$regex_code/g) {
        ...
        if (defined $1 && $1 ne '') {
            ...
            ######################################
                ...
                # hot keywords
                elsif (exists $HString_no_empty_string{$stringID} && $HString_no_empty_string{$stringID} =~ /\b(?:password|pwd|user(?:name)?|uid|auth|db|database|account)\b\s*[=:](?:\s|\\n|\\)*["'](.+?)["']/i) {
                    my $match = $1;
                    if ($match =~ /\S/ && $match ne '"' && $match ne "'") {
