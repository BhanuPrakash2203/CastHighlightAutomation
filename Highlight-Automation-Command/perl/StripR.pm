...
sub StripR($$$$)
{
    ...
    configureLocalTraces('StripR', $options); # traces_filter_line
    my $stripRTiming = new Timing ('StripR', Timing->isSelectedTiming ('Strip'));
    $stripRTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $stripRTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    $stripRTiming->dump('StripR') if ($b_timing_strip); # timing_filter_line
    return 0;
}
