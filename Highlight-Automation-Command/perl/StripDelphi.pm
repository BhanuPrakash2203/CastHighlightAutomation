...
sub parseLineComment($) {
	...
	print STDERR "Unterminated comment for sub parseLineComment !!!\n";
...
sub parseMultilineLineComment($) {
	...
	print STDERR "Unterminated comment for sub parseMultilineLineComment!!!\n";
	return 1;
}
...
#-----------------
# kinds: 
#  -  Simple quote string  '   ' merged with template string
#  -  Template string      '   '\n#13#10
#                        + '   '
#
# Escape :
...
sub StripDelphi($$$$)
{
    ...
    configureLocalTraces('StripDelphi', $options); # traces_filter_line
    my $stripDelphiTiming = new Timing ('StripDelphi', Timing->isSelectedTiming ('Strip'));
    $stripDelphiTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $stripDelphiTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    $stripDelphiTiming->dump('StripDelphi') if ($b_timing_strip); # timing_filter_line
    return 0;
}
