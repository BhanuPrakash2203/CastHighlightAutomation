...
sub parseIdentifier($) {
    ...
    # literal strings `` backslash \ will be ignored...
	while ( $$TEXT =~ /\G(\\\\|\\|\\`|`|"|'|\\\n|[^\\`"']*)/gc) {
		$spaces = $blank->($1);
		$COMMENT .= $spaces;
		$CODE_padding .= $spaces;
...
sub StripMySQL($$$$)
{
    ...
    configureLocalTraces('StripMySQL', $options); # traces_filter_line
    my $stripMySQLTiming = new Timing ('StripMySQL', Timing->isSelectedTiming ('Strip'));
    $stripMySQLTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $stripMySQLTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    if ( $err gt 0) {
     ...
     if ($CODE !~ /\S/s ) {
      return Erreurs::FatalError( Erreurs::ABORT_CAUSE_NOT_ENOUGH_CODE, $couples, "No MySQL found");
    }
    ...
    $stripMySQLTiming->dump('StripMySQL') if ($b_timing_strip); # timing_filter_line
    return 0;
}
