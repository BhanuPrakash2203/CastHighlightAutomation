...
sub ProcessFileList($$$) {
  ...
  if (defined $options->{'--KeywordScan'}) {
    ...
    # CloudReady::projectFiles::setMasterTechno($analyseur);
    # }
    # KeywordScan detection in project files will occur after sources files analysis.
  ...
  for my $fichier (@tabFiles) {
    ...
    $localStatus |= AnalyseFile($fichier, $options, $analyseur, $globalContext);
...
sub AnalyseFile($$$$)
{
  my ($fichier, $options, $analyzer, $globalContext ) = @_ ;
  Erreurs::SetCurrentFilenameTrace($fichier);
  AnalyseOptions::GetProgression($options)->ProgressBeginFile( $fichier);
  my $status = AnalyseUtil::AnalyseFile($fichier, $options, $analyzer, $globalContext, \&AnalyseFileInternal);
  return $status;
}
...
sub AnalyseFileInternal($$$$)
{
  my ($originalFilename, $options, $analyseur, $globalContext) = @_ ;
  my $couples ; 
