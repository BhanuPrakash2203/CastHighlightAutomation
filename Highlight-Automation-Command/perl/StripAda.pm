...
sub StripAda($$$$)
{
    ...
    configureLocalTraces('StripAda', $options); # traces_filter_line
    my $StripAdaTiming = new Timing ('StripAda', Timing->isSelectedTiming ('Strip'));
    $StripAdaTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $StripAdaTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    $StripAdaTiming->dump('StripAda') if ($b_timing_strip); # timing_filter_line
    return 0;
}
