...
sub StripGo($$$$)
{
    ...
    configureLocalTraces('StripGo', $options); # traces_filter_line
    my $stripGoTiming = new Timing ('StripGo', Timing->isSelectedTiming ('Strip'));
    $stripGoTiming->markTimeAndPrint('--init--') if ($b_timing_strip); # timing_filter_line
    
    my $err = parseCode(\$vue->{'text'}, $options);
    
    $stripGoTiming->markTimeAndPrint('separer_code_commentaire_chaine') if ($b_timing_strip); # timing_filter_line
    ...
    if ( $err gt 0) {
     ...
     if ($CODE !~ /\S/s ) {
      return Erreurs::FatalError( Erreurs::ABORT_CAUSE_NOT_ENOUGH_CODE, $couples, "No Go found");
    }
    ...
    $stripGoTiming->dump('StripGo') if ($b_timing_strip); # timing_filter_line
    return 0;
}
